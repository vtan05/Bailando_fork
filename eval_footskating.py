...
from smplx_fk import do_smplxfk, ax_from_6v, ax_to_6v, SMPLX_Skeleton
...
def calc_foot_skating_ratio(model_xp):
    # assert len(data.shape) == 2
    ...
    model_xp = torch.from_numpy(model_xp).float()
...
if __name__ == '__main__':
    ...
    test_dir = r"/data/van/Dance/Bailando_new/experiments/actor_critic/eval/pkl/ep000010"
    ...
    for file in os.listdir(test_dir):
        ...
        data = np.load(filepath, allow_pickle=True).item()['pred_position'][:, :] 
        data = np.array(data).reshape(-1, 22, 3)
