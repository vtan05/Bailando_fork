...
def write2json_original(dances, dance_names, config, expdir, epoch):
    ...
    for i in tqdm(range(len(dances)),desc='Generating Jsons'):
        ...
        dances[i] = dances[i].reshape(num_poses, 21, 3)
    ...
    if config.data.name == "aist":
        ...
        for i in range(len(results)):
            ...
            # print(np_dance.shape)
                ...
                #     np_dance += mean_pose
                root = np_dance[:, :3]
                np_dance = np_dance + np.tile(root, (1, 21))
                np_dance[:, :3] = root
            ...
            # np_dance_trans[:, 8] = np_dance2[:, 0] root
            ...
            # np_dance_trans[:, 19] = np_dance2[:, 11] rtoes
            # np_dance_trans[:, 20] = np_dance2[:, 11] rtoes
            # np_dance_trans[:, 21] = np_dance2[:, 8] rankle

            # np_dance_trans[:, 22] = np_dance2[:, 10] ltoes
            # np_dance_trans[:, 23] = np_dance2[:, 10] ltoes
            # np_dance_trans[:, 24] = np_dance2[:, 7] lankle

            # Visual 2D skeleton keypoints
            np_dance_trans[:, 0]  = np_dance2[:, 11]  # neck
            np_dance_trans[:, 1]  = np_dance2[:, 6]   # chest 

            # left arm
            np_dance_trans[:, 2]  = np_dance2[:, 15]  # lshoulder
            np_dance_trans[:, 3]  = np_dance2[:, 17]  # lelbow
            np_dance_trans[:, 4]  = np_dance2[:, 19]  # lwrist

            # right arm
            np_dance_trans[:, 5]  = np_dance2[:, 16]  # rshoulder
            np_dance_trans[:, 6]  = np_dance2[:, 18]  # relbow
            np_dance_trans[:, 7]  = np_dance2[:, 20]  # rwrist
            ...
            np_dance_trans[:, 15] = np_dance2[:, 14]  # head
            np_dance_trans[:, 16] = np_dance2[:, 14]  # head
            np_dance_trans[:, 17] = np_dance2[:, 14]  # head
            np_dance_trans[:, 18] = np_dance2[:, 14]  # head

            # right toes/foot repeated
            np_dance_trans[:, 19] = np_dance2[:, 10]  # rfoot
            np_dance_trans[:, 20] = np_dance2[:, 10]  # rfoot
            np_dance_trans[:, 21] = np_dance2[:, 8]   # rankle

            # left toes/foot repeated
            np_dance_trans[:, 22] = np_dance2[:, 9]   # lfoot
            np_dance_trans[:, 23] = np_dance2[:, 9]   # lfoot
            np_dance_trans[:, 24] = np_dance2[:, 7]   # lankle
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            if not rotmat:
                ...
                np_dance = np_dance - np.tile(root, (1, 21))  # Calculate relative offset with respect to root
                np_dance[:, :3] = root
...
def check_data_distribution(data_dir, interval=240, rotmat=False):
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            # np_dance = np_dance.reshape(-1, 21, 3)
...
#             b, c = np_dance.shape
#         else:
#             np_dance = np_dance[:, :21]
#             b, c, _ = np_dance.shape
#         # print(np_dance.shape)
        
#         np_dance = np_dance.reshape([b, 21, 3])
#         # b = min(b, 900)
#         np_dance = np_dance[:b]
...
#         # np_dance2 = np_dance[:, :, :2] / 2 - 0.5
#         # np_dance2[:, :, 1] = np_dance2[:, :, 1]
#         np_dance2 = np_dance.reshape(b, 21, 3)
#         np_dance2[:, :, 1:] *= -1
